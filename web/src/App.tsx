...
import {localizeActivityText} from './activityLabels';
      ...
      <main className="workspace-main">
        {workspaceView === 'workspace' ? (
          <div className="workspace-utility-bar">
            <button className="icon-button mobile-only" onClick={() => setMobileSidebarOpen(true)} aria-label="展开导航">
              <Menu size={19} />
            </button>
            {!sidebarOpen && (
              <button className="icon-button desktop-only" onClick={() => setSidebarOpen(true)} aria-label="展开导航">
          ...
          <header className="workspace-header">
            <div className="workspace-title-row">
              ...
              <div className="workspace-title-copy">
                <strong>{workspaceView === 'settings' ? '设置' : '生成产物'}</strong>
        ...
        {workspaceView === 'workspace' && <div className="composer-zone">
          {loadError && (
            <div className="inline-error" role="alert">
              <span>{loadError}</span>
              <button onClick={() => setLoadError('')} aria-label="关闭错误提示"><X size={15} /></button>
            </div>
          )}
          ...
          <div className={`composer ${chat.busy ? 'is-busy' : ''}`}>
                ...
                {workspaceUploads.map((file) => (
                  <span className="composer-attachment" key={file.path} title={file.path}>
                    ...
                    <button
                      ...
                      aria-label={`从本条消息中移除 ${file.name}`}
                    >
                      <X size={12} />
                    </button>
                  </span>
                ))}
                {uploadingFiles && <span className="composer-uploading">正在上传…</span>}
              </div>
            )}
            <div className="composer-controls">
              ...
              <button
                ...
                title={selectedSessionId ? '上传文件到工作区' : '请先创建或选择项目'}
              >
                <Plus size={20} />
              </button>
    ...
    <>
      ...
      <aside className={`sidebar ${open ? 'is-open' : 'is-collapsed'} ${mobileOpen ? 'is-mobile-open' : ''}`}>
        ...
        <nav className="primary-nav" aria-label="主导航">
          ...
          <button className={activeView === 'artifacts' ? 'is-active' : ''} onClick={onArtifacts}><Files size={17} /><span>生成产物</span></button>
          ...
          <div className="session-list">
            {sessions.map((session) => (
              ...
              >
                ...
                <button className="session-delete" onClick={() => onDelete(session)} aria-label={`删除项目：${sessionTitle(session)}`} title="删除项目">
                  <Trash2 size={14} />
                </button>
              </div>
            ))}
        ...
        <div className="sidebar-footer">
          ...
          <div><strong>本地工作区</strong><small>本地 · ViMax</small></div>
        </div>
      </aside>
    </>
  );
...
function ActivityRow({message}: {message: Message}) {
  ...
  const text = localizeActivityText(message.text);
  const detail = stage.toLowerCase() === text.toLowerCase() ? text : [stage, text].filter(Boolean).join(' · ');
...
function ThinkingRow({messages}: {messages: Message[]}) {
  ...
  return (
    <div className="thinking-row">
      ...
      <span>{running ? `${humanize(running.tool || 'ViMax')} · ${localizeActivityText(running.text)}` : 'ViMax 正在思考'}</span>
    </div>
  );
}
...
function SlashCommandMenu({matches, contextPercent, onSelect}: {matches: SlashCommandMatch[]; contextPercent: number; onSelect: (command: string) => void}) {
  return (
    <div className="slash-command-menu" role="listbox" aria-label="斜杠命令">
      ...
      )) : <span className="slash-command-empty">没有匹配的命令</span>}
    </div>
  );
}

const CONFIG_SECTIONS: Array<{key: keyof AgentConfig['sections']; title: string; description: string}> = [
  {key: 'llm', title: '智能体语言模型', description: '用于创作规划、工具选择和对话'},
  {key: 'image', title: '图像生成', description: '用于生成角色图、关键帧和镜头画面'},
  {key: 'video', title: '视频生成', description: '用于生成镜头片段和最终视频'},
  {key: 'embedding', title: '向量模型', description: '可选，用于小说内容检索'},
  {key: 'reranker', title: '重排模型', description: '可选，用于小说检索结果排序'},
];
    ...
    <section className="settings-view">
      <header>
        ...
        <div className="settings-save-group">
          {status && <span className={status === '已保存' ? 'is-saved' : 'is-error'}>{status}</span>}
          <button className="settings-save" onClick={() => void save()} disabled={saving}>
            <Save size={15} />{saving ? '正在保存…' : '保存'}
          </button>
        </div>
      </header>
  ...
  return (
    <section className="config-section">
      ...
      <div className="config-fields">
        ...
        <label><span>模型名称</span><input value={value.model} onChange={(event) => onChange('model', event.target.value)} /></label>
        <label className="config-field-wide"><span>接口地址</span><input value={value.base_url} onChange={(event) => onChange('base_url', event.target.value)} inputMode="url" /></label>
        <label className="config-field-wide">
          <span>API 密钥 <i className={value.has_api_key ? 'is-configured' : ''}>{value.has_api_key ? '已配置' : '未配置'}</i></span>
          <input type="password" value={value.api_key} onChange={(event) => onChange('api_key', event.target.value)} placeholder={value.has_api_key ? '留空以保留当前密钥' : '请输入 API 密钥'} autoComplete="off" />
        </label>
      </div>
    </section>
  );
    ...
    <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onCancel()}>
      <section className="delete-dialog" role="dialog" aria-modal="true" aria-labelledby="delete-project-title">
        ...
        <div className="dialog-copy">
          ...
          <p>项目「<strong>{sessionTitle(session)}</strong>」及其生成文件将被永久删除。</p>
        </div>
        <div className="dialog-actions">
          <button onClick={onCancel} disabled={deleting} autoFocus>取消</button>
          <button className="danger" onClick={onConfirm} disabled={deleting}>{deleting ? '正在删除…' : '删除'}</button>
        </div>
      </section>
    </div>
  );
    ...
    <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onCancel()}>
      <form className="project-dialog" role="dialog" aria-modal="true" aria-labelledby="new-project-title" onSubmit={(event) => {
        ...
        <div className="dialog-actions">
          ...
          <button type="submit" className="primary" disabled={creating || !name.trim()}>{creating ? '正在创建…' : '创建'}</button>
        </div>
      </form>
    </div>
  );
...
function stageLabel(stage: string) {
  const labels: Record<string, string> = {
    created: '已创建',
    narrative_planning: '正在规划',
    narrative_planned: '规划已就绪',
    novel_planning: '正在规划小说',
    novel_planned: '小说规划已就绪',
    rendering: '正在渲染',
    rendered: '渲染已完成',
    error: '需要处理',
  };
