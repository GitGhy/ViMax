...
export function ArtifactsView({session, artifacts}: {session?: SessionSummary; artifacts: Artifact[]}) {
  ...
  return (
    <section className="artifacts-view">
      <header>
        <div><span>生成文件</span><h1>{session ? projectTitle(session) : '生成产物'}</h1></div>
        <span className="artifact-file-count">{jsonArtifacts.length} 份文档 · {mediaArtifacts.length} 个媒体文件</span>
      </header>
      <div className="artifact-view-switcher" role="tablist" aria-label="产物类型">
        {(['all', 'documents', 'visuals'] as const).map((option) => (
          ...
          >
            {option === 'all' ? '全部' : option === 'documents' ? '文档' : '图像与视频'}
          </button>
        ))}
      </div>
      {!session ? (
        <ArtifactsEmpty title="请选择项目" detail="完成规划后，项目文件会显示在这里" />
      ) : (
        <>
          {(view === 'all' || view === 'documents') && (
            <section className="artifact-content-section">
              {view === 'all' && <SectionHeading title="项目产物" detail="文档及其关联的图像与视频" />}
              {jsonArtifacts.length === 0 ? (
                <ArtifactsEmpty title="暂无规划文档" detail="完成规划后，可在这里查看剧本、角色和分镜等内容" />
              ) : (
                <div className="artifact-browser">
                  <nav className="artifact-document-list" aria-label="生成的规划文档">
                  ...
                  <article className={`artifact-document ${view === 'all' ? 'has-related-visuals' : ''}`}>
                    {selected && (
                      <header>
                        <div><span>文档详情</span><h2>{friendlyArtifactTitle(selected)}</h2></div>
    ...
    <section className="artifact-visuals">
      {artifacts.length > 0 ? (
        <div className="render-grid">
          {artifacts.map((artifact) => (
            <article key={artifact.path} className="render-item">
              <button className="render-media" onClick={() => onPreview(artifact)} aria-label={`预览${visualArtifactLabel(artifact)}`}>
                {artifact.kind === 'image'
        ...
        <div className="renders-empty">
          <Film size={24} />
          <strong>暂无图像或视频</strong>
          <span>渲染过程中，生成的图像和视频会显示在这里</span>
        </div>
      )}
    </section>
  );
...
function RelatedVisuals({artifacts, onPreview}: {artifacts: Artifact[]; onPreview: (artifact: Artifact) => void}) {
  return (
    <aside className="artifact-related-visuals" aria-label="关联图像与视频">
      <header>
        <div><span>关联图像与视频</span><strong>{artifacts.length}</strong></div>
        <small>{artifacts.length > 0 ? '点击预览' : '等待渲染'}</small>
      </header>
      {artifacts.length > 0 ? (
        ...
        <div className="related-visual-empty">
          <ImageIcon size={19} />
          <span>与本文档对应的图像和视频片段会显示在这里。</span>
        </div>
      )}
    </aside>
  );
...
export function StoryboardPanel({open, artifacts, activeRenderStage, onClose, onCountChange}: {
  ...
  return (
    <aside className={`storyboard-panel ${open ? 'is-open' : ''}`}>
      <header>
        <div><strong>分镜</strong><span>{previews.length > 0 ? `第 ${index + 1} / ${previews.length} 个` : '预览'}</span></div>
        <button className="icon-button" onClick={onClose} aria-label="关闭分镜预览"><X size={18} /></button>
      </header>
      <section className="render-readiness" aria-label="渲染准备状态">
        <div className="render-readiness-summary">
          <strong>{readiness.readyToRender ? '已就绪，可以渲染' : '规划尚未完成'}</strong>
        </div>
        <div className="render-checkpoints">
          <ReadinessCheckpoint
            label="分镜"
            detail={`${readiness.storyboards.count} 个镜头`}
        ...
        <div className="render-stage-heading">
          <span>渲染</span>
          <small>{activeRenderStage ? '正在生成' : readiness.render.finalVideo.status === 'ready' ? '已完成' : readiness.render.started ? '部分完成' : '未开始'}</small>
        </div>
        <div className="render-checkpoints">
          <ReadinessCheckpoint
            label="关键帧"
          ...
          <ReadinessCheckpoint
            label="最终视频"
            detail={readiness.render.finalVideo.status === 'ready' ? '已就绪' : '待生成'}
...
function StatusLight({status, generating = false}: {status: ReadinessStatus; generating?: boolean}) {
  const labels: Record<ReadinessStatus, string> = {ready: '已就绪', partial: '部分完成', missing: '尚未就绪', inactive: '未开始'};
  return <i className={`status-light is-${status} ${generating ? 'is-generating' : ''}`} aria-label={generating ? `${labels[status]}，正在生成` : labels[status]} />;
}
...
function StructuredNested({label, value, depth}: {label: string; value: JsonValue; depth: number}) {
  if (depth > 5) return <StructuredField label={label} value="更多详细信息" />;
  if (Array.isArray(value)) {
...
function visualArtifactLabel(artifact: Artifact): string {
  ...
  const labels: Record<string, string> = {
    first_frame: '首帧', last_frame: '尾帧', video: '视频片段', final_video: '最终视频',
    front: '正面照', side: '侧面照', back: '背面照', transition_video: '转场视频', new_camera_image: '新机位画面',
  };
  const stem = artifact.name.replace(/\.[^.]+$/, '');
  const base = Object.hasOwn(labels, stem) ? labels[stem] : artifact.name
    .replace(/\.[^.]+$/, '')
    .replace(/[_-]+/g, ' ')
    .replace(/\b\w/g, (character) => character.toUpperCase());
  if (portrait) return `${portrait[1]} · ${base}`;
  const context = [];
  if (scene !== undefined) context.push(`场景 ${Number(scene) + 1}`);
  if (shot !== undefined) context.push(`镜头 ${Number(shot) + 1}`);
